...
**Version 0.1 — Draft** (cleaned reference copy; link syntax repaired from a
mangled rich-text export, plus a §6 cross-reference to the §11 `okf_version`
exception; content otherwise unchanged.
upstream: GoogleCloudPlatform/knowledge-catalog `okf/SPEC.md`,
upstream-commit: ee67a5ca27044ebe7c38385f5b6cffc2305a9c1a)
...
Index files contain no frontmatter (with the single exception of the
bundle-root `index.md` declaring `okf_version` — see §11). The body uses
one or more sections, each grouping concepts under a heading:
